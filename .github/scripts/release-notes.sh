...
previous="${2:-}"
repository="${GITHUB_REPOSITORY:-OWNER/gits}"
...
\`\`\`sh
dir=\$(mktemp -d) &&
archive="gits-\$(uname -s)-\$(uname -m).tar.gz" &&
...
tar -xzf "\$dir/\$archive" -C "\$dir" gits &&
mkdir -p ~/.local/bin &&
install -m 755 "\$dir/gits" ~/.local/bin/gits
[ -n "\$dir" ] && rm -rf "\$dir"
command -v gits
\`\`\`
...
\`gits\` in that directory, and it REPLACES a \`gits\` that is already there.

**\`install\` does not put \`~/.local/bin\` on your path.** \`command -v gits\` must
...
On Windows, take \`gits-Windows-x86_64.zip\` (or \`gits-Windows-arm64.zip\` for
an ARM machine), check it against the \`.sha256\` file beside it, and put the
\`gits.exe\` that it holds in a directory on your \`PATH\`.
